...
; Transmitting switches the receiver off, and it comes back late - a single read
; has been measured blocking 470 ms while the bus was carrying frames. Measured
; 3000 ms between processed lever frames with app replies on, 496 ms with them
; off. So the count of transmissions is what matters: an app answer is held and
; sent in the same write as the next dash answer, halving them.
(def app-pend false)
(def pend-dev 0)
(def pend-src 0)
(def pend-reg 0)
...
(defun nb-app-frame (dev src cmd reg len)
    (cond
        ...
        ((= cmd 0x01) (let ((n (if (> len 0) (bufget-u8 uart-buf 4) 2))) {
            ...
            ; Hold it for the next dash answer so both go out in one write. A
            ; request arriving while one is already held is dropped; the app
            ; re-asks, and that costs nothing compared to a transmission.
...
(defun main () {
        ...
        (if (= model 2) { ; Slave: code server only, model stays switchable over CAN
            ...
            (def combo (array-create 80)) ; a dash answer plus an app answer
