...
- **Warning lamps** above the speed: a red engine lamp for any controller fault, and an
  ...
  and the slowest wheel, measured against the ADC app's own traction control limit, so it
  needs more than one motor to see anything. The lamp holds for two seconds, because a
  slip is over before you could look down
